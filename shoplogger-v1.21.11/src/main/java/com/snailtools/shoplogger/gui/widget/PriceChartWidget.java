 ...
 * draws simple 1px line segments between points via repeated DrawContext.fill
 * calls, same technique used elsewhere for lightweight in-game plots.
 * Mirrors 404.html's two-world (Firefly/Honeybee) price-history charts.
 */
public final class PriceChartWidget {
	...
	public static void draw(DrawContext context, TextRenderer tr, int x, int y, int w, int h, String title,
		...
		int boxY = y + titleH;
		int boxH = h - titleH;
		context.drawTextWithShadow(tr, title, x, y, 0xFFD9C89A);

		context.fill(x, boxY, x + w, boxY + boxH, 0x662A3A2F);
		drawRectOutline(context, x, boxY, w, boxH, 0xFF33453A);

		if (history == null || history.isEmpty()) {
			context.drawCenteredTextWithShadow(tr, "No price history yet", x + w / 2, boxY + boxH / 2 - 4, 0xFF8FA593);
			return;
		}
		...
		drawAxisRow(context, tr, labelRightX, plotX, plotY, plotW, maxVal);
		drawAxisRow(context, tr, labelRightX, plotX, plotY + plotH / 2, plotW, maxVal / 2);
		drawAxisRow(context, tr, labelRightX, plotX, plotY + plotH, plotW, 0);
		...
		context.drawTextWithShadow(tr, "FF", x + w - 64, boxY + 2, FIREFLY_COLOR);
		context.fill(x + w - 44, boxY + 2, x + w - 38, boxY + 8, HONEYBEE_COLOR);
		context.drawTextWithShadow(tr, "HB", x + w - 34, boxY + 2, HONEYBEE_COLOR);
	}

	/** Faint gridline plus its numeric value, right-aligned just left of the plot area. */
	private static void drawAxisRow(DrawContext context, TextRenderer tr, int labelRightX, int plotX, int rowY, int plotW, double value) {
		context.fill(plotX, rowY, plotX + plotW, rowY + 1, 0x22FFFFFF);
		String label = fmtAxis(value);
		int labelW = tr.getWidth(label);
		context.drawTextWithShadow(tr, label, labelRightX - labelW, rowY - 4, 0xFF8FA593);
	}
	...
	private static void drawSeries(DrawContext context, List<String> dates, Map<String, Double> series,
	...
	private static void drawLineSegment(DrawContext context, int x1, int y1, int x2, int y2, int color) {
	...
	private static void drawRectOutline(DrawContext context, int x, int y, int w, int h, int color) {
