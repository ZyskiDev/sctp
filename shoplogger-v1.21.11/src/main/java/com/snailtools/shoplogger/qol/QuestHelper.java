...
public class QuestHelper {
    ...
    public void onScreenRender(Screen screen, DrawContext gui, int mouseX, int mouseY, float delta) {
        ...
        if (minecraft.player == null || minecraft.textRenderer == null || isInButterflyGardenLevel()) return;
    ...
    public void onMouseEvent(long window, int button, int action) {
        ...
        MinecraftClient minecraft = MinecraftClient.getInstance();
        if (minecraft.player == null || !isQuestScreen(minecraft.currentScreen) || isInButterflyGardenLevel()) return;

        QuestDisplay questDisplay = questDisplayForCurrentPaper(minecraft);
        List<Text> lines = visibleQuestLines(questDisplay);
        updateControlBounds(minecraft, questDisplay);
        updateScaleControlBounds(minecraft.currentScreen);
    ...
    public void onHUDRender(DrawContext graphics) {
        ...
        QuestDisplay questDisplay = questDisplayForCurrentPaper(minecraft);
        boolean showToggle = isQuestScreen(minecraft.currentScreen);
        List<Text> lines = visibleQuestLines(questDisplay);
        updateControlBounds(minecraft, questDisplay);

        if (questHelperVisible && !lines.isEmpty()) {
            renderQuestTable(graphics, minecraft, minecraft.player.getOffHandStack(), questDisplay, showToggle);
        } else if (showToggle) {
    ...
    private boolean isCurrentPlayerQuestPaper(ItemStack stack) {
        MinecraftClient minecraft = MinecraftClient.getInstance();
        if (minecraft.player == null || stack == null || stack.isEmpty() || !stack.isOf(Items.PAPER)) return false;

        NbtString questUuid = Util.getCustomDataVar(stack, QUEST_UUID_DATA, NbtString.class);
    ...
    private QuestDisplay questDisplayForCurrentPaper(MinecraftClient minecraft) {
        if (minecraft.player == null) return QuestDisplay.EMPTY;

        ItemStack questPaper = minecraft.player.getOffHandStack();
        if (!isCurrentPlayerQuestPaper(questPaper)) return QuestDisplay.EMPTY;

        return questDisplay(questPaper);
    }

    private QuestLayout calculateLayout(MinecraftClient minecraft, QuestDisplay questDisplay) {
        List<Text> lines = visibleQuestLines(questDisplay);
        int maxTextWidth = 22 + minecraft.textRenderer.getWidth(questDisplay.title());
        for (Text line : lines) {
        ...
        int headerControlsWidth = PANEL_PADDING * 2 + 22 + minecraft.textRenderer.getWidth(questDisplay.title()) + TOGGLE_MARGIN + (TOGGLE_SIZE * 2) + 3;
    ...
    private void updateControlBounds(MinecraftClient minecraft, QuestDisplay questDisplay) {
        QuestLayout layout = calculateLayout(minecraft, questDisplay);
    ...
    private void updateScaleControlBounds(Screen screen) {
        ...
        scaleSliderX = MinecraftClient.getInstance().getWindow().getScaledWidth() - PANEL_MARGIN_RIGHT - SCALE_SLIDER_WIDTH;
        scaleSliderY = 4;
    }

    private void renderQuestTable(DrawContext graphics, MinecraftClient minecraft, ItemStack questPaper, QuestDisplay questDisplay, boolean showToggle) {
        List<Text> lines = visibleQuestLines(questDisplay);
        QuestLayout layout = calculateLayout(minecraft, questDisplay);
        ...
        graphics.drawItem(questPaper, titleX + 2, panelY + 1);
        int titleTextX = titleX + 22;
        graphics.drawText(minecraft.textRenderer, questDisplay.title(), titleTextX, titleY, TEXT_COLOR, true);
        if (showToggle) {
    ...
    private void renderQuestLine(DrawContext graphics, MinecraftClient minecraft, Text line, int leftX, int rightX, int y, int fallbackColor) {
        String text = line.getString();
        int colonIndex = text.indexOf(':');
        if (colonIndex < 0 || colonIndex == text.length() - 1) {
            graphics.drawText(minecraft.textRenderer, withFallbackColor(line, fallbackColor), leftX, y, fallbackColor, false);
            return;
        }
        ...
        Text label = withFallbackColor(styledText(line, labelText), fallbackColor);
        Text value = withFallbackColor(styledText(line, valueText), fallbackColor);
        int valueWidth = minecraft.textRenderer.getWidth(value);
        graphics.drawText(minecraft.textRenderer, label, leftX, y, fallbackColor, false);
        graphics.drawText(minecraft.textRenderer, value, rightX - valueWidth, y, fallbackColor, false);
    }

    private int questLineWidth(MinecraftClient minecraft, Text line) {
        String text = line.getString();
        int colonIndex = text.indexOf(':');
        if (colonIndex < 0 || colonIndex == text.length() - 1) {
            return minecraft.textRenderer.getWidth(line);
        }
        ...
        return minecraft.textRenderer.getWidth(styledText(line, labelText)) + 8 + minecraft.textRenderer.getWidth(styledText(line, valueText));
    }

    private void renderCollapsedToggle(DrawContext graphics, MinecraftClient minecraft, QuestDisplay questDisplay) {
        QuestLayout layout = calculateLayout(minecraft, questDisplay);
    ...
    private void renderToggle(DrawContext graphics, int x, int y) {
    ...
    private void renderCompactToggle(DrawContext graphics, int x, int y) {
        graphics.fill(x, y, x + TOGGLE_SIZE, y + TOGGLE_SIZE, CONTROL_BORDER_COLOR);
    ...
    private void renderQuestControls(DrawContext gui, MinecraftClient minecraft, int mouseX, int mouseY) {
        ...
        gui.drawCenteredTextWithShadow(minecraft.textRenderer, label, scaleSliderX + SCALE_SLIDER_WIDTH / 2, scaleSliderY + 3, TEXT_COLOR);
    ...
    private void updatePanelOffsetFromMouse(MinecraftClient minecraft, QuestDisplay questDisplay, int mouseX, int mouseY) {
    ...
    private List<Text> visibleQuestLines(QuestDisplay questDisplay) {
        if (!questCompact) return questDisplay.lines();

        List<Text> candidateLines = compactDelimitedLines(questDisplay.lines());
        List<Text> visibleLines = new ArrayList<>();
        for (Text line : candidateLines) {
    ...
    private List<Text> compactDelimitedLines(List<Text> lines) {
    ...
    private boolean isSeparatorLine(Text line) {
        String text = line.getString();
    ...
    private boolean isCompactHiddenLine(Text line) {
        String text = line.getString();
        int colonIndex = text.indexOf(':');
    ...
    private Text styledText(Text source, String text) {
        return Text.literal(text).setStyle(source.getStyle());
    }

    private Text withFallbackColor(Text component, int fallbackColor) {
        if (component.getStyle().getColor() != null) {
            return component;
        }
        return component.copy().setStyle(component.getStyle().withColor(TextColor.fromRgb(fallbackColor & 0xFFFFFF)));
    }

    private record QuestLayout(int panelX, int panelY, int panelWidth, int panelHeight) {
    }

    private record QuestDisplay(Text title, List<Text> lines) {
        private static final QuestDisplay EMPTY = new QuestDisplay(Text.literal("Quest"), List.of());
    }
}
