...
public final class RareRentalHighlighter {
	...
	public void onScreenRender(Screen screen, GuiGraphicsExtractor gui, int mouseX, int mouseY, float delta) {
		...
		for (Slot slot : containerScreen.getMenu().slots) {
			if (slot.container == client.player.getInventory()) continue; // only the shop's own container side
			ItemStack stack = slot.getItem();
			if (stack.isEmpty()) continue;
			if (!names.contains(MatchUtil.alphaOnly(stack.getHoverName().getString()))) continue;
	...
	private void drawSlotBorder(GuiGraphicsExtractor gui, int x, int y) {
