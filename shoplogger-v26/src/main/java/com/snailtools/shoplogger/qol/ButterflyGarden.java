...
public class ButterflyGarden {
    ...
    public void tick() {
        ...
        if (Minecraft.getInstance().screen == null && exchangeSelling) {
            exchangeSelling = false;
        }
        ...
        if (!(minecraft.screen instanceof AbstractContainerScreen<?> screen) || !isButterflyExchangeScreen(screen)) {
            exchangeSelling = false;
            currentExchangeType = null;
            clearExchangeWait();
            return;
    ...
    public void onMouseEvent(long window, int button, int action) {
        ...
        if (action == 1 && Minecraft.getInstance().screen instanceof AbstractContainerScreen<?> screen && isButterflyExchangeScreen(screen)) {
    ...
    public void onHUDRender(GuiGraphicsExtractor graphics) {
        ...
        boolean showToggle = isButterflyExchangeScreen(minecraft.screen);
    ...
    private HudLayout calculateHudLayout(Minecraft minecraft, List<ButterflyCount> counts) {
        int totalPoints = 0;
        int maxNameWidth = minecraft.font.width("Butterflies");
